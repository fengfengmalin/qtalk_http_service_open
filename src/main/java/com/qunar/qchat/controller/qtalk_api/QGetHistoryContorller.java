...
import com.qunar.qchat.service.QchatGetMsg;
import com.qunar.qchat.utils.CookieUtil;
...
public class QGetHistoryContorller {
    ...
    @Autowired
    private QchatGetMsg qchatGetMsg;
    ...
    public JsonResult<?> getMsg(HttpServletRequest request,@RequestBody String json) {
        ...
        try{
            if (json.length() > 0) {
                ...
                Map<String,Object> cookies = CookieUtil.getUserbyCookie(request);
    ...
    private void get_muc_domain(Set<String> sets,String user,String host,Map<String,Object> maps){
      ...
      for (String domain:sets){
            if (domain.equals("conference.ejabhost2") || domain.equals("ejabhost2")){

                mucTimes.addAll(qchatGetMsg.selectMucTime(user,host));
    ...
    {
        List<QGetMucMsg> smsglist = new ArrayList<>();
        if (domain.equals("ejabhost2") || domain.equals("conference.ejabhost2")){
            if (time == -1){
                List<QMucTime> mucTimes = qchatGetMsg.get_qchat_muctime(user,host);
                for (QMucTime mucTime : mucTimes){
                    List<QGetMucMsg>  mucMsgs = qchatGetMsg.get_qchat_mucmsgs(mucTime.getMuc_name(),">","asc",1L,((Double.valueOf(mucTime.getDate()))/1000));
                    smsglist.addAll(mucMsgs);
                }
            }else {
                smsglist.addAll(qchatGetMsg.selectLocalDomainMucHistory(user, host, time, num));
            }
        }else{
            if (time == -1){
